...
graph TD
    ...
    subgraph "Backend Service (Python Container)"
        Gateway --> Auth[认证模块 (OAuth2/LDAP)]
        Gateway --> Manager[业务逻辑层]
        
        subgraph "RAG Logic"
            Manager --> Ingestion[文档解析 (PyMuPDF)]
            Manager --> Retrieval[混合检索器]
            Manager --> ChatEngine[对话引擎]
    ...
    subgraph "Data Storage"
        ...
        FileSys[本地文件存储 (Uploads)]
    end

    subgraph "Model Service (Ollama)"
        LLM[Qwen 2.5 (生成)]
        Embed[BGE-M3 (向量化)]
    end
...
contextDiagram
    ...
    User -->> System : 1. 提问/查询
    User -->> System : 2. 上传文档
    Admin -->> System : 3. 用户/部门管理
    
    System -->> LDAP : 4. 账号验证
    System -->> Ollama : 5. 文本向量化 (Embed)
    System -->> DB : 6. 向量检索与存取
    System -->> Ollama : 7. 上下文推理 (Chat)
    
    System -->> User : 8. 返回答案 + 引用来源
```
...
## 📄 License
@Copyright 本项目允许个人和企业免费使用，但是严禁使用于商业用途。